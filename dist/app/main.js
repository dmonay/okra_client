(function () {
  ...
  app.config([
    ...
    function ($mdThemingProvider) {
      ...
      var okraGreenAccent = $mdThemingProvider.extendPalette('green', { 'A200': '#99b742' });
      ...
      $mdThemingProvider.theme('default').primaryPalette('okraGreen').accentPalette('okraGreenAccent');
    }
  ]);
  ...
  app.run([
    ...
    '$window',
    'session',
    function ($http, $location, $rootScope, $state, $window, session) {
  ...
  app.constant('okraAPI', {
    ...
    getOrganizations: 'https://evening-river-9352.herokuapp.com/get/orgs/all/',
    createTree: 'https://evening-river-9352.herokuapp.com/create/tree/',
    getTreesInOrg: 'https://evening-river-9352.herokuapp.com/get/trees/',
    getSingleTreeInOrg: 'https://evening-river-9352.herokuapp.com/get/trees/',
    createObjective: 'https://evening-river-9352.herokuapp.com/create/objective/',
    updateObjective: 'https://evening-river-9352.herokuapp.com/update/objective/properties/',
    createKeyResult: 'https://evening-river-9352.herokuapp.com/create/kr/',
...
(function () {
  ...
  AddOrganizationModalController.$inject = [
    '$scope',
    'OrganizationFactory',
    '$mdDialog'
  ];
...
(function () {
  ...
  function OrganizationController($scope, $mdDialog, TreeFactory, $stateParams, OrganizationFactory) {
    ...
    TreeFactory.getTrees(vm.organization).then(function (response) {
    ...
    vm.openAddTreeModal = function ($event) {
      $mdDialog.show({
        ...
        controllerAs: 'modal',
        locals: { organization: vm.organization }
      }).then(function (response) {
...
(function () {
  ...
  OrganizationSelectionController.$inject = [
    '$scope',
    '$mdDialog',
    'OrganizationFactory',
    'hardCoded',
...
(function () {
  ...
  function okCollapse($animate) {
    ...
    function linkFunc(scope, iElement, iAttrs) {
      var thisNode = angular.element(document.getElementById(scope.linkedTo)), allNodes = scope.allLinkedNodes;
      iElement.bind('click', function () {
        var isCollapsed = thisNode.next().hasClass('collapse');
        //hide all nodes beneath the node we chose
        var node, i = allNodes.indexOf(scope.linkedTo) + 1;
...
(function () {
  ...
  /**
     ...
     */
  app.directive('okDrawConnection', [
    'jsPlumbDefaults',
    '$window',
    function (jsPlumbDefaults, $window) {
      return {
        restrict: 'A',
        link: linkFunc
      };
...
(function () {
  ...
  MissionStatementModalController.$inject = [
    '$scope',
    'OrganizationFactory',
    '$mdDialog',
    'missionStatement',
...
(function () {
  ...
  function NodeModalDashboardController($scope, TreeFactory, $mdDialog, editMode, nodeType, node, tree, parentNode, secondaryParentNode) {
    var modal = this;
    ...
    modal.createNode = function () {
      ...
      if (modal.nodeForm.$valid) {
        ...
        TreeFactory[apiMethod](tree, modal.node, parentNode, secondParentNode).then(function (response) {
  ...
  NodeModalDashboardController.$inject = [
    '$scope',
    'TreeFactory',
    '$mdDialog',
    'editMode',
...
(function () {
  ...
  function TreeController($scope, $mdDialog, TreeFactory, $stateParams, $filter, $timeout) {
    ...
    var vm = this, treeId = $filter('okDecrypt')($stateParams.treeIdEnc);
    function getTree() {
      TreeFactory.getSingleTree($stateParams.organization, treeId).then(function (response) {
    ...
    vm.doubleTapComplete = function (apiMethod, node, parentNode, secondParentNode) {
      ...
      TreeFactory[apiMethod](vm.tree, node, parentNode, secondParentNode).then(function (response) {
    ...
    vm.openMissionStatementModal = function ($event) {
      $mdDialog.show({
        ...
        controllerAs: 'modal',
        locals: { missionStatement: vm.tree.Mission }
      }).then(function (response) {
    ...
    vm.openNodeModalDashboard = function ($event, editMode, nodeType, node, parentNode, secondaryParentNode) {
      $mdDialog.show({
        ...
        if (response) {
          var node = response.node, parentNode = response.parentNode;
          if (editMode) {
            var objectiveIndex;
            if (nodeType === 'Objective') {
              ...
              vm.tree.Objectives[objectiveIndex].KeyResults[keyResultIndex].Name = node.Name;
              vm.tree.Objectives[objectiveIndex].KeyResults[keyResultIndex].Body = node.Body;
              vm.tree.Objectives[objectiveIndex].KeyResults[keyResultIndex].Priority = node.Priority;
              vm.tree.Objectives[objectiveIndex].KeyResults[keyResultIndex].Completed = node.Completed;
            } else {
  ...
  function TreeFactory($http, okraAPI) {
    var TreeFactoryAPI = {
        ...
        updateKeyResult: function (tree, keyResult, objective) {
          ...
          var url = okraAPI.updateKeyResult + tree.OrgName + '/' + tree.Id + '/' + objective.Id + '/' + keyResultIndex;
          return $http.post(url, {
        ...
        updateTask: function (tree, task, objective, keyResult) {
          var keyResultIndex = parseFloat(keyResult.Id[2]) - 1, taskIndex = parseFloat(task.Id[4]) - 1, url = okraAPI.updateTask + tree.OrgName + '/' + tree.Id + '/' + objective.Id + '/' + keyResultIndex + '/' + taskIndex;
          return $http.post(url, {
            taskname: task.Name,
            taskbody: task.Body,
            priority: task.Priority,
